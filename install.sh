#!/bin/zsh
# iSlippi installer for players. Paste this into Terminal:
#
#   curl -fsSL https://raw.githubusercontent.com/TheAndersMadsen/islippi/main/install.sh | zsh
#
# It checks the Mac, gets Apple's developer tools if needed, downloads (or updates) iSlippi into ~/iSlippi, asks for your
# Melee disc image, builds the app from it on this Mac, puts iSlippi in Applications and opens it. Run it again to update.
set -euo pipefail
yellow() { printf '\n\033[1;33m%s\033[0m\n' "$1"; }
fail() { printf '\n\033[1;31m%s\033[0m\n' "$1" >&2; exit 1; }
DIR="${ISLIPPI_DIR:-$HOME/iSlippi}"

[[ "$(uname)" == "Darwin" && "$(uname -m)" == "arm64" ]] || fail "iSlippi needs a Mac with Apple silicon (M1 or newer)."

if ! xcode-select -p >/dev/null 2>&1; then
  yellow "iSlippi needs Apple's free developer tools."
...
if [[ -d "$DIR/.git" ]]; then
  yellow "Updating iSlippi in $DIR"
  git -C "$DIR" pull --ff-only --quiet
else
  yellow "Downloading iSlippi into $DIR"
...
rm -rf "$DEST/iSlippi.app"
ditto "$DIR/dist/iSlippi.app" "$DEST/iSlippi.app"
yellow "iSlippi is in $DEST. Opening it now."
open "$DEST/iSlippi.app" --args --iso "$ISO" --choose-disc
